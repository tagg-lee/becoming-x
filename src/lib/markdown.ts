import { Marked, type Tokens } from 'marked';

const CALLOUT_TYPES: Record<string, { icon: string; label: string }> = {
	NOTE: {
		icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8Zm8-6.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13ZM6.5 7.75A.75.75 0 0 1 7.25 7h1a.75.75 0 0 1 .75.75v2.75h.25a.75.75 0 0 1 0 1.5h-2a.75.75 0 0 1 0-1.5h.25v-2h-.25a.75.75 0 0 1-.75-.75ZM8 6a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z"/></svg>',
		label: 'Note',
	},
	TIP: {
		icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8 8 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"/></svg>',
		label: 'Tip',
	},
	WARNING: {
		icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"><path d="M6.457 1.047c.659-1.234 2.427-1.234 3.086 0l6.082 11.378A1.75 1.75 0 0 1 14.082 15H1.918a1.75 1.75 0 0 1-1.543-2.575Zm1.763.707a.25.25 0 0 0-.44 0L1.698 13.132a.25.25 0 0 0 .22.368h12.164a.25.25 0 0 0 .22-.368Zm.53 3.996v2.5a.75.75 0 0 1-1.5 0v-2.5a.75.75 0 0 1 1.5 0ZM9 11a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z"/></svg>',
		label: 'Warning',
	},
	IMPORTANT: {
		icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"><path d="M0 1.75C0 .784.784 0 1.75 0h12.5C15.216 0 16 .784 16 1.75v9.5A1.75 1.75 0 0 1 14.25 13H8.06l-2.573 2.573A1.458 1.458 0 0 1 3 14.543V13H1.75A1.75 1.75 0 0 1 0 11.25Zm1.75-.25a.25.25 0 0 0-.25.25v9.5c0 .138.112.25.25.25h2a.75.75 0 0 1 .75.75v2.19l2.72-2.72a.749.749 0 0 1 .53-.22h6.5a.25.25 0 0 0 .25-.25v-9.5a.25.25 0 0 0-.25-.25Zm7 2.25v2.5a.75.75 0 0 1-1.5 0v-2.5a.75.75 0 0 1 1.5 0ZM9 9a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z"/></svg>',
		label: 'Important',
	},
};

function slugify(text: string): string {
	return text
		.toLowerCase()
		.trim()
		.replace(/[\s]+/g, '-')
		.replace(/[^\p{L}\p{N}\-]/gu, '')
		.replace(/-+/g, '-')
		.replace(/^-|-$/g, '');
}

const instance = new Marked();

instance.use({
	renderer: {
		heading({ tokens, depth }: Tokens.Heading): string {
			const text = this.parser.parseInline(tokens);
			const rawText = tokens
				.map((t) => ('text' in t ? t.text : t.raw))
				.join('');
			const id = slugify(rawText);

			if (depth >= 2 && depth <= 6) {
				return `<h${depth} id="${id}">${text}<a href="#${id}" class="heading-anchor" aria-hidden="true">#</a></h${depth}>\n`;
			}
			return `<h${depth}>${text}</h${depth}>\n`;
		},

		blockquote({ tokens }: Tokens.Blockquote): string {
			const body = this.parser.parse(tokens);

			// Check for callout syntax: [!TYPE]
			const calloutMatch = body.match(
				/^\s*<p>\s*\[!(NOTE|TIP|WARNING|IMPORTANT)\]\s*/i,
			);
			if (calloutMatch) {
				const type = calloutMatch[1].toUpperCase();
				const config = CALLOUT_TYPES[type];
				if (config) {
					const content = body.replace(
						/^\s*<p>\s*\[!(NOTE|TIP|WARNING|IMPORTANT)\]\s*/i,
						'<p>',
					);
					return `<div class="callout callout-${type.toLowerCase()}"><div class="callout-header">${config.icon} ${config.label}</div>${content}</div>\n`;
				}
			}

			return `<blockquote>${body}</blockquote>\n`;
		},

		code({ text, lang }: Tokens.Code): string {
			const escaped = text
				.replace(/&/g, '&amp;')
				.replace(/</g, '&lt;')
				.replace(/>/g, '&gt;')
				.replace(/"/g, '&quot;');
			const langAttr = lang ? ` class="language-${lang}"` : '';

			if (lang) {
				return `<div class="code-block-wrapper"><div class="code-lang-label">${lang}</div><pre><code${langAttr}>${escaped}</code></pre></div>\n`;
			}
			return `<pre><code${langAttr}>${escaped}</code></pre>\n`;
		},
	},
});

export interface TocHeading {
	depth: number;
	text: string;
	id: string;
}

export function extractHeadings(input: string): TocHeading[] {
	const tokens = instance.lexer(input);
	const headings: TocHeading[] = [];

	for (const token of tokens) {
		if (token.type === 'heading' && token.depth >= 2 && token.depth <= 6) {
			const rawText = token.tokens
				?.map((t: any) => ('text' in t ? t.text : t.raw))
				.join('') ?? token.text;
			headings.push({
				depth: token.depth,
				text: rawText,
				id: slugify(rawText),
			});
		}
	}

	return headings;
}

export function extractHeadingsFromHtml(html: string): TocHeading[] {
	const headings: TocHeading[] = [];
	const regex = /<h([2-6])\s+id="([^"]*)"[^>]*>(.*?)<a\s+href/g;
	let match;

	while ((match = regex.exec(html)) !== null) {
		const depth = parseInt(match[1], 10);
		const id = match[2];
		const text = match[3].replace(/<[^>]*>/g, '').trim();
		headings.push({ depth, text, id });
	}

	return headings;
}

export async function marked(input: string): Promise<string> {
	return await instance.parse(input);
}
